...
const FailedContent: React.FC = () => {
  const [failedJobs, setJobs] = useState<FailedJobs>({
    total_failed: 0,
    jobs: []
  });
  ...
  const handleJobDeleted = (id: string) => {
    const filteredJobs = failedJobs.jobs.filter(
      job => job.payload.args[0].job_id !== id
    );
    setJobs({ total_failed: failedJobs.total_failed - 1, jobs: filteredJobs });
  };

  const handleRetry = () => {
    fetch(`${BASE_PATH}/api/retry_all`, {
      method: "POST"
    }).then(response => {
      if (response.ok) {
        setFirstJob(0);
  ...
  const atMaxJob = () =>
    firstJob + stepBy >= failedJobs.total_failed ||
    failedJobs.total_failed === 0;
  const atMinJob = () => firstJob === 0;

  useEffect(() => {
    setLoading(true);
    fetch(`${BASE_PATH}/api/failed?from_job=${firstJob}`)
  ...
  const renderControls = () => {
    ...
    return (
      <Grid container justify="center" spacing={2}>
        <Button onClick={handlePrevClicked} disabled={atMinJob() || isLoading}>
          Previous
        </Button>
        <Button disabled>Page {Math.ceil((firstJob + 1) / stepBy)}</Button>
        <Button onClick={handleNextClicked} disabled={atMaxJob() || isLoading}>
          Next
        </Button>
      </Grid>
    );
  ...
  return (
    <div className={classes.tabContent}>
      <Typography variant="h6">
        ...
        >
          Clear Failed <DeleteIcon />
        </Button>
        <Button className={classes.spaceLeft} onClick={handleRetry}>
          Retry All <RefreshIcon />{" "}
        </Button>
      </Typography>
      ...
      <Grid className={classes.gridRoot} container spacing={2}>
        ...
        <Grid item xs={12}>
          <List>
            {failedJobs.jobs.map(item => (
              <FailedRow
                job={item}
                onJobDeleted={handleJobDeleted}
                onJobRetried={handleJobDeleted}
                key={item.payload.args[0].job_id}
              />
            ))}
          </List>
        </Grid>
