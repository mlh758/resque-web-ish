...
const FailedRow: React.FC<FailedProps> = ({
  job,
  onJobDeleted,
  onJobRetried
}) => {
  ...
  const handleDeleteJob = () => {
    ...
    setDeleting(true);
    fetch(`${BASE_PATH}/api/failed_job`, request(jobID, "DELETE")).then(
      response => {
        if (response.ok) {
          onJobDeleted(jobID);
        } else {
  ...
  const handleRetryJob = () => {
    const jobID = job.payload.args[0].job_id;
    setRetrying(true);
    fetch(`${BASE_PATH}/api/retry_job`, request(jobID, "POST")).then(
      response => {
        if (response.ok) {
          onJobRetried(jobID);
        } else {
      ...
      <ExpansionPanel>
        ...
        <ExpansionPanelDetails>
          <div className={classes.panelContents}>
            <div>
              {job.backtrace
                .slice(0, Math.min(job.backtrace.length, 10))
                .map((traceLine, idx) => {
                  return (
                    <Typography
                      variant="body2"
                      className={classes.traceSpacing}
                      key={idx}
                    >
                      {traceLine}
                    </Typography>
