...
const QueueContent: React.FC<ContentProp> = ({ queue }) => {
  ...
  const handleQueueClear = () => {
    fetch(`${BASE_PATH}/api/queue/${queue}`, {
      method: "DELETE"
    }).then(response => {
      if (response.ok) {
  ...
  const atMaxJob = () =>
    firstJob > queueJobs.total_jobs || queueJobs.total_jobs === 0;
  const atMinJob = () => firstJob === 0;

  useEffect(() => {
    fetch(`${BASE_PATH}/api/queue/${queue}?from_job=${firstJob}`)
  ...
  const renderControls = () => {
    ...
    return (
      <Grid container justify="center" spacing={2}>
        <Button onClick={handlePrevClicked} disabled={atMinJob()}>
          Previous
        </Button>
        <Button disabled>Page {Math.ceil((firstJob + 1) / 10)}</Button>
        <Button onClick={handleNextClicked} disabled={atMaxJob()}>
          Next
        </Button>
      </Grid>
    );
  ...
  return (
    <div className={classes.tabContent}>
      ...
      <List>{queueJobs.jobs.map(job => renderJobItem(job))}</List>
    </div>
  );
};
